...
- **完成日期**: 2026-02-05
- **主要成就**:
  - [x] **Task-优化**: 提升图片清晰度与导出质量。
    - 上传压缩：最大宽度从 1500px 提升至 2500px，JPEG 质量从 0.7 提升至 0.9。
    - PDF 导入：采用**混合无损方案** (Hybrid Lossless)，PDF 页面转为 PNG 格式，彻底消除压缩噪点。
    - PDF 导出：html2canvas 采样率从 2x 提升至 3x (300DPI)。
  - [x] **Task-视觉**: 付款凭单样式标准化。
    - 统一全部文字颜色为 `text-slate-900`，消除打印色差。
    - 统一段落字重架构：标签使用 `Bold`，填写内容使用 `Normal`，金额保留 `Bold`。
...
- **[2026-02-05] UI 交互与细节优化**
    ...
    - **全局显隐控制**: 
        - 在 `SettingsStore` 中扩展了 `showFileFields` 配置，支持按模式(`payment`/`invoice`)独立控制。
        - 在凭单设置面板（未选中文件时）添加了“文件输入框”全局开关。
        - 实现了 `FileItem` 根据此全局设置动态显示/隐藏底部输入区域。
    - **交互优化**: 在凭单设置面板的 `用途摘要` 和 `总金额` 标签旁增加了 Info 提示图标，Hover 时显示关于手动修改逻辑的说明信息。
    - **文案优化**:
        - 将“表头可见”重命名为“显示付款凭单”，表意更清晰。
    - 将“底部输入框”重命名为“文件下金额/用途/备注”，明确控制范围。
    - 解决了引入 Tooltip 组件导致开发服务器未及时响应的热更新白屏问题。

- **[2026-03-04] 导出功能与移动端上传能力**
    - **ZIP 导出归档**: 实现了 `useExportZip`，允许将画板中的图片按项目元数据（金额、类别）规范命名打包为 ZIP 文件。增加了对工作区过滤。
    - **移动端上传 (图片/PDF)**: 集成 `peerjs` 实现不需要后端的局域网内 P2P 高速照片传输。
      - 构建了 `/mobile-upload` 这个专为手机打造的极简上传页。
      - 并在用户的建议下，除了照片选取外，集成了既有的 PDF 解析组件至移动端网页，支持传输前拆分页并压缩发往电脑端。
    - **UI 修正**: 
      - 加入了确实的全局右侧属性面板显示/隐藏收起按钮，并在移动设备中默认不展现右侧面板。
      - 修改首次进入且处于“发票报销”模式时的默认布局为 vertical (上下分栏)。
      - 对齐导出 PDF 时各个项目的名字与原生一致。

- **[2026-03-25] PDF 发票 CJK 字体兼容性修复**
    - **问题**: 导入特定 PDF 电子发票后，发票的关键信息（包括金额、商品明细等）无法显示并在 Canvas 上留白。
    - **原因**: 发票 PDF 内使用如 `SimSun`（宋体）等未内嵌的中日韩字体，由于缺少 CMap 以及运行环境的本地字体库缺失而导致解析失败。
    - **修复**:
      - 静态映射表离线支持：将 `pdfjs-dist` 内置的 CMap 和标准字体物理放置到 `public/pdf-cmaps/` 与 `public/pdf-fonts/` 中，以不依赖外网且避免受其他拦截器缓存干扰的形式提供给库。
      - 解析配置覆盖：在 `pdfjsLib.getDocument` 中设置对应资源的前缀并开启 `cMapPacked`。
      - 环境降级补丁：通过对宿主侧的原生 `window.FontFace` 进行 API 拦截 (Monkey Patch)，自动将寻找 `SimSun` 的调用请求映射代理至 Mac 环境内置中文字体系列（如 `Songti SC`, `PingFang SC`）。徹底避免了无对应字体的设备上出现的 PDF 渲染静默失败。

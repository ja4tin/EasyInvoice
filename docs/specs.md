* | **项目名称** | *智能报销排版工具 (EasyInvoice)*                             |
  ...
  **画布规格**：固定 **A4 标准尺寸** (210mm x 297mm)，背景白色。
  ...
  - **技术策略**：**High-DPI Snapshot (高清容器快照)**。
    ...
    - **ZIP 打包下载 (V1.1 New)**：
      - **能力集成**：整合 PDF 导出与原始高清图聚合功能，将所有内容塞入纯前端生成的 `.zip` 包。
      - **约束规则**：原始高清图按照 `序号_发票用途_单据金额.[后缀]` 的统一规则命名，以备直接存档。
  ...
  | **高级功能** | 导出包含原图和 PDF 的 ZIP 归档包       | **P1 (中)** | V1.1         |
  | **高级功能** | 跨端 (手机到电脑) 局域网 P2P 扫码传图  | **P1 (中)** | V1.1         |
  | **高级功能** | OCR 自动识别金额                       | P2 (低)     | V2.0         |
  | **高级功能** | 多币种支持                             | P3 (低)     | V2.0         |
  | **高级功能** | 云端同步/团队协作                      | P3 (低)     | V3.0         |
  ...
  4. **`ExportService` (导出服务)**：
     - 封装 `html2canvas`, `jspdf` 以及 `jszip`，负责锁定 DOM、缩放渲染、生成 PDF 文件流或者合成并触发含原文件的 ZIP 压缩包下载。
  5. **`PeerService` (P2P通信服务 - V1.1)**：
     - 借由 `peerjs` 实现手机端扫码后与桌面端浏览器的无缝直连（WebRTC 流通信），并在收到图片文件 Blob 后通知 Store 自动追加至左侧列表中。
  ...
  | **状态管理** | **Zustand**                                                  | 管理全局的文件序列、金额计算、UI 状态，以及新加入的 WebRTC 通信状态监测。 |
  ...
  | **网络通信** | **peerjs** (V1.1 新增)                                       | 无需搭配企业后端，纯前端提供高效 P2P WebRTC 链接实现手机扫码即入。 |
  | 工具库       | `decimal.js`, `dayjs`, `jszip` (导出压缩), `qrcode.react` (生成二维码) |                                                              |

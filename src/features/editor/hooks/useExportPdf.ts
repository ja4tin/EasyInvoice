...
export function useExportPdf() {
  const { appMode, invoiceLayout } = useSettingsStore(
  ...
  const generatePdf = useCallback(async () => {
    ...
    for (let i = 0; i < pages.length; i++) {
      ...
      // 强制特定容器为白底 (Fix: 502)
      const whiteBgElements = clonedPage.querySelectorAll(
        ".pdf-export-bg-white",
      );
      ...
      // 强制移除边框 (Fix: 503)
      const borderNoneElements = clonedPage.querySelectorAll(
        ".pdf-export-border-none",
      );
      ...
      // 移除隐藏空输入框容器的逻辑，让其实际显示状态完全由全局设置控制
      ...
      for (let j = 0; j < originalInputs.length; j++) {
        const originalInput = originalInputs[j] as
          | HTMLInputElement
          | HTMLTextAreaElement;
        const clonedInput = clonedInputs[j] as
          | HTMLInputElement
          | HTMLTextAreaElement;
